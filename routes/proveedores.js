...
router.get('/nuevo', (req, res) => {
  res.render('proveedores/form', { proveedor: {}, categorias: CATEGORIAS_GASTO, accion: '/proveedores', error: null });
});
...
router.get('/:id/editar', async (req, res, next) => {
  try {
    ...
    res.render('proveedores/form', { proveedor: rows[0], categorias: CATEGORIAS_GASTO, accion: `/proveedores/${req.params.id}`, error: null });
  } catch (err) { next(err); }
});
  ...
  try {
    ...
    if (!p.nombre || !p.nombre.trim()) {
      return res.render('proveedores/form', {
        ...
        categorias: CATEGORIAS_GASTO,
        accion: `/proveedores/${req.params.id}`,
        error: 'Falta el nombre del proveedor.',
      });
    }
    await pool.query(
...
router.post('/:id/eliminar', async (req, res, next) => {
  try {
    ...
    if (cantidad > 0) {
      ...
      return res.redirect('/proveedores?error=' + encodeURIComponent(msg));
    }
