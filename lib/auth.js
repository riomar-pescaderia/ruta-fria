...
const MODULOS = ['clientes', 'articulos', 'prospectos', 'ventas', 'cuenta_corriente', 'compras', 'informes', 'stock'];
...
async function crearUsuario({ username, password, nombre, esAdmin, accesos, permisos }) {
  ...
  const { rows } = await pool.query(
    `insert into usuarios
      (username, password_hash, password_visible, nombre, es_admin, acceso_clientes, acceso_articulos, acceso_compras, acceso_informes, acceso_ventas, acceso_prospectos, acceso_stock, acceso_cuenta_corriente, permiso_editar_confirmadas)
...
async function actualizarPermisos(id, { esAdmin, accesos, permisos }) {
  ...
  const p = permisos || {};
  await pool.query(
    `update usuarios set es_admin=$1, acceso_clientes=$2, acceso_articulos=$3, acceso_compras=$4, acceso_informes=$5, acceso_ventas=$6, acceso_prospectos=$7, acceso_stock=$8, acceso_cuenta_corriente=$9, permiso_editar_confirmadas=$10
     where id = $11`,
    [!!esAdmin, !!a.clientes, !!a.articulos, !!a.compras, !!a.informes, !!a.ventas, !!a.prospectos, !!a.stock, !!a.cuenta_corriente, !!p.editarConfirmadas, id]
  );
  return { ok: true };
}
