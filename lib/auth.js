...
const MODULOS = ['clientes', 'articulos', 'compras', 'gastos', 'ventas', 'prospectos'];
...
async function crearUsuario({ username, password, nombre, esAdmin, accesos, permisos }) {
  ...
  const { rows } = await pool.query(
    `insert into usuarios
      (username, password_hash, nombre, es_admin, acceso_clientes, acceso_articulos, acceso_compras, acceso_gastos, acceso_ventas, acceso_prospectos, permiso_editar_confirmadas)
...
async function actualizarPermisos(id, { esAdmin, accesos, permisos }) {
  ...
  const p = permisos || {};
  await pool.query(
    `update usuarios set es_admin=$1, acceso_clientes=$2, acceso_articulos=$3, acceso_compras=$4, acceso_gastos=$5, acceso_ventas=$6, acceso_prospectos=$7, permiso_editar_confirmadas=$8
     where id = $9`,
    [!!esAdmin, !!a.clientes, !!a.articulos, !!a.compras, !!a.gastos, !!a.ventas, !!a.prospectos, !!p.editarConfirmadas, id]
  );
  return { ok: true };
}
