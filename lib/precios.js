...
function calcularPrecios(articulo, config) {
  ...
  const precioEfectivo = costoConCargos * (1 + margenPct / 100);
  const precioLista = precioEfectivo * (1 + recargoListaPct / 100);

  return {
    costoConCargos: round2(costoConCargos),
    precioEfectivo: round2(precioEfectivo),
    precioLista: round2(precioLista),
  };
}
