...
//       -> + 35% -> precio_sugerido (para el listado de precios en PDF:
//          el precio que le sugerimos al cliente mayorista que le ponga a
//          la reventa de cada producto — no es un precio propio, es
//          información de referencia para él)
//
...
function calcularPrecios(articulo, config) {
  ...
  // Precio sugerido de reventa: siempre 35% sobre el precio en efectivo ya
  // redondeado (no sobre el de lista) — es un porcentaje fijo del negocio,
  // no una config editable como el recargo por medio de pago.
  const precioSugerido = Math.round(precioEfectivo * 1.35);
