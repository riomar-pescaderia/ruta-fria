...
function calcularPrecios(articulo, config) {
  ...
  // Un precio cargado a mano (columna precio_manual, ver db/schema.sql)
  // pisa la fórmula de costo+margen — 0 es un valor válido a propósito
  // (marca el artículo como "Sin stock" en el listado de precios en PDF,
  // ver routes/articulos.js), por eso se compara con null/undefined y no
  // con un simple "if (precio_manual)".
  const tienePrecioManual = articulo.precio_manual !== null && articulo.precio_manual !== undefined && articulo.precio_manual !== '';
  const precioEfectivo = tienePrecioManual
    ? round2(Number(articulo.precio_manual))
    : redondearA100(costoConCargos * (1 + margenPct / 100));
