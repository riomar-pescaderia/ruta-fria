...
// Modo vigente de origen del stock — "automatico" (ventas/compras lo
// mueven solas) o "planilla" (una hoja externa manda, ver
// lib/stockPlanilla.js). Se puede pasar una conexión con una transacción
// abierta (para leerlo de forma consistente dentro de una operación de
// venta/compra) o nada, para usar el pool normal.
async function obtenerConfigStock(client) {
  const ejecutor = client || pool;
  const { rows } = await ejecutor.query('select * from stock_config where id = 1');
  return rows[0] || { modo: 'automatico', planilla_url: null, planilla_nombre: null };
}

module.exports = { registrarMovimiento, obtenerConfigStock };
